...
import { ITEMS, removeItem } from "../../storage/economy.js";
...
export async function execute(message, args) {
  ...
  adjustBalance(message.author.id, item.sell);
  return message.reply({ embeds: [baseEmbed(COLORS.success).setTitle(`${"\u{1F9FE}"} Sold`).setDescription(`${item.emoji} **${item.name}** for ${EMOJIS.coin} **${item.sell.toLocaleString()}** (50% of price).`)] });
}
