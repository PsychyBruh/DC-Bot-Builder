...
export const name = "balance-leaderboard";
export const description = "Top balances";
export const usage = "!balance-leaderboard";
export const category = "leaderboard";

export async function execute(message) {
  const top = getTopUsers("balance", 10);
  ...
  const lines = await Promise.all(top.map(async (u, i) => {
    ...
    return `${medal} **${name}** — ${EMOJIS.coin} ${(u.balance || 0).toLocaleString()}`;
  }));
  const embed = baseEmbed(COLORS.gold)
    .setTitle(`${EMOJIS.coin} Richest Players`)
